...
def fluxDensityCocoon(t, nu, umax, Ei, ts, q, Mej, n0, p, epsE, epsB, 
    ...
    for i in range(len(t)):
        ...
        args = (amu, ate, aus, ar, nu[i], n0, p, epsE, epsB, ksiN)
...
def dP(theta, amu, ate, aus, ar, nu, n0, p, epsE, epsB, ksiN):
    ...
    us = ((mu-amu[ia])*aus[ib] + (amu[ib]-mu)*aus[ia]) / (amu[ib]-amu[ia])
    r = ((mu-amu[ia])*ar[ib] + (amu[ib]-mu)*ar[ia]) / (amu[ib]-amu[ia])

    uNR = 1.51186
    if us > uNR:
        u = math.sqrt(0.5*(us*us-1))
    else:
        u = 3.0/math.sqrt(16/(us*us)+7.0)
